    ...
    content: [
...

      {
        table: {
          ...
          body: [
            ...
            ['NIP', `: ${data.ul_2.replace(/^.*?:/, '').trim()}`],
            [
              'Pangkat / Gol. Ruang',
              `: ${data.ul_3.replace(/^.*?:/, '').trim()}`,
            ],
            ['Jabatan', `: ${data.ul_4.replace(/^.*?:/, '').trim()}`],
            ['Unit Kerja', `: ${data.ul_5.replace(/^.*?:/, '').trim()}`],
          ],
        },
        layout: 'noBorders',
        margin: [0, 0, 0, 20],
...

      {
        table: {
          widths: ['auto', '*'],
          body: [
            ['Nama', `: ${data.ul_6.replace(/^.*?:/, '').trim()}`],
            [
              'Tempat, Tanggal Lahir',
              `: ${data.ul_7.replace(/^.*?:/, '').trim()}`,
            ],
            ['Kelas', `: ${data.ul_8.replace(/^.*?:/, '').trim()}`],
            ['NISN', `: ${data.ul_9.replace(/^.*?:/, '').trim()}`],
            ['Nama Ibu', `: ${data.ul_10.replace(/^.*?:/, '').trim()}`],
            ['Nama Ayah', `: ${data.ul_11.replace(/^.*?:/, '').trim()}`],
          ],
        },
        layout: 'noBorders',
        margin: [0, 0, 0, 20],
